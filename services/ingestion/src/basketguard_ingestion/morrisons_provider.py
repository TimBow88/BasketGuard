...
MORRISONS_FEATURE_FLAG = "BASKETGUARD_ENABLE_MORRISONS_SCRAPER"
MORRISONS_PARSER_VERSION = "morrisons-html-v1"


class MorrisonsParseError(ValueError):
    pass


@dataclass(frozen=True)
class MorrisonsScraperConfig:
...
class MorrisonsProductPageParser:
    retailer = "Morrisons"
    ...
    def parse(
        ...
        if not name:
            raise MorrisonsParseError("Missing product title")
        ...
        raw_snapshot = RawProductSnapshot(
            ...
            parser_version=MORRISONS_PARSER_VERSION,
...
class MorrisonsIngestionProvider:
    provider_name = "morrisons"

    def __init__(self, config: MorrisonsScraperConfig) -> None:
        self.config = config
        self.parser = MorrisonsProductPageParser()

    def collect(self) -> IngestionJobResult:
        if not self._enabled():
            return IngestionJobResult(
                provider_name=self.provider_name,
                job_type="morrisons_allowlisted_product_collection",
                status="failed",
                retailer="Morrisons",
                ...
                notes=(
                    f"Morrisons live scraping is disabled. Set {MORRISONS_FEATURE_FLAG}=1 "
        ...
        for index, url in enumerate(self.config.allowlisted_urls):
            ...
            except (FetchError, MorrisonsParseError, UnitNormalisationError, ValueError) as error:
        ...
        return IngestionJobResult(
            ...
            notes="Live Morrisons collection ran against explicit allowlisted URLs.",
        )

    def _enabled(self) -> bool:
        return self.config.enabled and os.environ.get(MORRISONS_FEATURE_FLAG) == "1"
...
def _parse_money(text: str | None) -> Decimal:
    if not text:
        raise MorrisonsParseError("Missing money value")
    ...
    raise MorrisonsParseError(f"Could not parse money value: {text!r}")


def _parse_unit_price(text: str | None) -> tuple[Decimal, str]:
    if not text:
        raise MorrisonsParseError("Missing unit price")
    match = re.search(
    ...
    if not match:
        raise MorrisonsParseError(f"Could not parse unit price: {text!r}")
...
def _error_code(error: Exception) -> str:
    ...
    if isinstance(error, MorrisonsParseError):
        return "parse_error"
