...
SAINSBURYS_FEATURE_FLAG = "BASKETGUARD_ENABLE_SAINSBURYS_SCRAPER"
SAINSBURYS_PARSER_VERSION = "sainsburys-html-v1"


class SainsburysParseError(ValueError):
    pass


@dataclass(frozen=True)
class SainsburysScraperConfig:
...
class SainsburysProductPageParser:
    ...
    def parse(
        ...
        if not name:
            raise SainsburysParseError("Missing product title")
        ...
        raw_snapshot = RawProductSnapshot(
            ...
            parser_version=SAINSBURYS_PARSER_VERSION,
...
class SainsburysIngestionProvider:
    provider_name = "sainsburys"

    def __init__(self, config: SainsburysScraperConfig) -> None:
        self.config = config
        self.parser = SainsburysProductPageParser()

    def collect(self) -> IngestionJobResult:
        if not self._enabled():
            return IngestionJobResult(
                provider_name=self.provider_name,
                job_type="sainsburys_allowlisted_product_collection",
                status="failed",
                retailer="Sainsbury's",
                ...
                notes=(
                    f"Sainsbury's live scraping is disabled. Set {SAINSBURYS_FEATURE_FLAG}=1 "
        ...
        for index, url in enumerate(self.config.allowlisted_urls):
            ...
            except (FetchError, SainsburysParseError, UnitNormalisationError, ValueError) as error:
        ...
        return IngestionJobResult(
            ...
            notes="Live Sainsbury's collection ran against explicit allowlisted URLs.",
        )

    def _enabled(self) -> bool:
        return self.config.enabled and os.environ.get(SAINSBURYS_FEATURE_FLAG) == "1"
...
def _parse_money(text: str | None) -> Decimal:
    if not text:
        raise SainsburysParseError("Missing money value")
    ...
    raise SainsburysParseError(f"Could not parse money value: {text!r}")


def _parse_unit_price(text: str | None) -> tuple[Decimal, str]:
    if not text:
        raise SainsburysParseError("Missing unit price")
    match = re.search(
    ...
    if not match:
        raise SainsburysParseError(f"Could not parse unit price: {text!r}")
...
def _error_code(error: Exception) -> str:
    ...
    if isinstance(error, SainsburysParseError):
        return "parse_error"
